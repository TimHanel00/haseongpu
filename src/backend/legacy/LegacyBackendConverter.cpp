...
namespace hase::internal::legacy
{
    using namespace hase::backend;
    using hase::backend::legacy::ExcitationUpdatePlan;
    ...
    ExcitationUpdatePlan excitationUpdatePlan(Simulation const& simulation, Assembly const& assembly)
    {
        ...
        return ExcitationUpdatePlan{std::move(active), std::move(domainCellIndices)};
    }
...
{
    using namespace hase::internal::legacy;

    ExcitationUpdatePlan::ExcitationUpdatePlan(
    ...
    std::vector<double> ExcitationUpdatePlan::values(ExcitationState const& state) const
    {
    ...
    void ExcitationUpdatePlan::apply(ExcitationState const& state, std::vector<double>& destination) const
    {
    ...
    {
        return std::move(convertWithUpdates(simulation).context);
    }

    LegacyBackendConversion LegacyBackendConverter::convertWithUpdates(Simulation const& simulation)
    {
        ...
        core::SimulationContext
            context{std::move(experiment), std::move(compute), std::move(mesh), std::move(result), std::move(run)};
        return {std::move(context), std::move(excitationPlan)};
    }
} // namespace hase::backend::legacy
