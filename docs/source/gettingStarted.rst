...
runtime dependency explicit in the deployment environment. If the Python wheel
should use a separately built HASE native runtime, build ``hase-cpp`` and the
helper targets in that CMake tree and set ``HASE_RUNTIME_DIR`` to the build tree
or install prefix before running Python.
