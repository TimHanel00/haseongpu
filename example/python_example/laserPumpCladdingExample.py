...
pump_diameter = 3
pump = {
    "s_abs": 0.778e-20,  # Absorption cross section in cm^2
    "s_ems": 0.195e-20,  # Emission cross section in cm^2
...
# Laser parameter
laser = {
    "s_abs": np.loadtxt("sigma_a.txt"),
    "s_ems": np.loadtxt("sigma_e.txt"),
    "l_abs": np.loadtxt("lambda_a.txt"),
    "l_ems": np.loadtxt("lambda_e.txt"),
    "l_res": 1000,
    "I": 1e6,
    "T": 1e-8,
    "wavelength": 1030e-9,
...
Ntot_gradient = np.zeros(crystal["levels"])
...
t = np.array([[x - 1 for x in y] for y in pt_data["t"]])  # index shift (Matlab -> python)
...
for i_p in range(p.shape[0]):
    ...
    pump["I"] = intensity * np.exp(
        -np.sqrt(p[i_p, 0] ** 2 / pump["ry"] ** 2 + p[i_p, 1] ** 2 / pump["rx"] ** 2) ** pump["exp"]
    )
    beta_crystal, beta_store, pulse, Ntot_gradient = beta_int3(
        beta_crystal, pulse, phy_const, crystal, steps, pump, mode, Ntot_gradient
    )
...
for i_p in range(p.shape[0]):
    for i_z in range(mesh_z):
        beta_cell[i_p, i_z] = (
                crystal["tfluo"]
                * (dndt_pump[i_p, i_z] - dndt_ASE[i_p, i_z])
                * (1 - np.exp(-time_t / crystal["tfluo"]))
                + beta_cell[i_p, i_z] * np.exp(-time_t / crystal["tfluo"])
        )

vtk_wedge("beta_cell_1.vtk", beta_cell, p, t_int, mesh_z, z_mesh)

################################ cladding #########################################
clad_number = np.int32(1)
...
for i_slice in range(1, timeslice_tot):
    ...
    z_2 = np.zeros(x_2.shape[0]) + z_mesh
    ...
    slice_time = slice_end - slice_start
    slice_times_series.append(slice_time)
    print(f"[TIME] Total time for timeslice {i_slice}: {slice_time:.3f} s")
