...
for relative in (
    ...
    if not isinstance(compute, dict):
        raise RuntimeError(f"{relative} does not contain a compute mapping")
    parallel_key = "parallel_mode" if "parallel_mode" in compute else "parallelMode"
    ranks_key = "n_per_node" if "n_per_node" in compute else "nPerNode"
    compute[parallel_key] = "mpi"
    compute[ranks_key] = ranks
    output = destination / relative.name
    output.write_text(yaml.safe_dump(config, sort_keys=False), encoding="utf-8")
    print(f"{output}: {parallel_key}=mpi, {ranks_key}={ranks}")
PY
