...
readonly campaign_venv=${source_root}/.venv-two-kernel-tuning
...
-DHASE_RUNTIME_DIR=${source_root}/build-two-kernel-tuning \
...
test -x "${source_root}/build-two-kernel-tuning/calcPhiASE"
grep -q '^HASE_ENABLE_ALPAKATUNE:BOOL=ON$' "${source_root}/build-two-kernel-tuning/CMakeCache.txt"
