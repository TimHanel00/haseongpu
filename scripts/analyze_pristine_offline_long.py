#!/usr/bin/env python3
"""Analyze the long pristine fixed-FrameSpec comparison."""
...
MODES = ("baseline_0", "offline_0", "offline_1", "baseline_1")
...
def main(argv=None):
    ...
    for mode in MODES:
        ...
        if summary["completed_steps"] != 3000 or summary["timed_out"] or not summary["finite"]:
        ...
        runs.append((mode, elapsed_seconds(directory / "time.txt")))
    ...
    report = [
        "# Long pristine baseline versus fixed offline FrameSpec",
        "",
        "After one unmeasured 100-step warm-up per binary, four measured runs shared one A100 allocation and executed 3,000 application steps with normal per-step openPMD snapshots. The `B-F-F-B` order counterbalances linear drift. Performance is external GNU `/usr/bin/time` wall clock; no internal or tuner instrumentation was enabled.",
        "",
    ...
    report.extend(
        [
            "",
            f"Original FrameSpec mean: {baseline_mean:.3f} s.",
            f"Fixed offline winners mean: {offline_mean:.3f} s.",
