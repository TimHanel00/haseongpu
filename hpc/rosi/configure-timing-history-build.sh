...
readonly tuned_venv=${source_root}/.venv-timing
...
-DHASE_RUNTIME_DIR=${source_root}/build-timing-history \
...
(
    cd "${source_root}"
    CMAKE_ARGS="${cmake_args}" "${tuned_venv}/bin/python3" -m pip install .
)

test -x "${source_root}/build-timing-history/calcPhiASE"
grep -q '^HASE_ENABLE_ALPAKATUNE:BOOL=ON$' "${source_root}/build-timing-history/CMakeCache.txt"
