...
Important Settings
------------------
These settings can be written through ``PhiASE`` configuration and are serialized into the openPMD compute metadata. The MPI process topology is controlled by how ``hase-cpp`` is launched, for example with ``mpiexec`` or a scheduler.
...
``nPerNode``
   ...
   that explicitly call ``mpiexec`` for the ``hase-cpp`` binary.
