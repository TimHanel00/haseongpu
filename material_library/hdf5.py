...
from .model import CrossSectionTable, Material, MaterialState
...
def read_material_library(path):
    with h5py.File(path, "r") as handle:
        ...
        for key, material_group in handle["materials"].items():
            ...
            material = Material(str(name), metadata=_read_metadata(material_group))
            for state_group in material_group["states"].values():
                ...
                state = MaterialState(
                    ...
                    bulkAttenuation=_read_quantity(state_group, "bulk_attenuation"),
