...
{
    using hase::transport::TransportReader;
...
namespace hase::transport
{
    ...
    TransportReader::TransportReader(io::Series& series, io::Iteration& iteration) : m_series(&series)
    {
        if(!iteration.containsAttribute("haseTransportVersion")
           || iteration.getAttribute("haseTransportVersion").get<std::string>() != "1.2")
        ...
        for(auto const& name : iteration.attributes())
        {
            if(name.starts_with(attributePrefix))
            {
                ...
                if(attribute.dtype == io::Datatype::VEC_STRING)
                    m_stringArrays.emplace(path, attribute.get<std::vector<std::string>>());
                else
