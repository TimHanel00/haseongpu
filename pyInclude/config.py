...
def install_command(selection: WizardSelection):
    ...
    return (
        ...
        'CMAKE_ARGS="$HASE_CONFIGURE_CMAKE_ARGS" python3 -m pip install .'
    )


def run_install(selection: WizardSelection):
    ...
    return subprocess.run([sys.executable, "-m", "pip", "install", "."], env=env, check=False).returncode
...
def main(argv=None):
    ...
    print(install_command(selection))
