...
class MaterialState:
    ...
    bulkAttenuation
        Non-negative passive attenuation coefficient with inverse-length unit.
    metadata
    ...
    bulkAttenuation: Quantity = Quantity(0.0, units.m**-1)
    """Passive intensity attenuation coefficient in inverse length."""
    metadata: Mapping[str, Any] = field(default_factory=dict)
...
class MaterialCondition:
    """Immutable material properties resolved for one simulation condition.

    Parameters
    ----------
    materialName
        Name of the originating :class:`Material`.
    temperature
        Selected absolute temperature, or ``None`` for an explicitly
        unknown-temperature reference.
    refractiveIndex, fluorescenceLifetime, crossSections, bulkAttenuation
    ...
    def __post_init__(self):
        ...
        if self.opticalAxis is not None:
            ...
            object.__setattr__(self, "opticalAxis", tuple(axis / np.linalg.norm(axis)))
        if density_si > 0.0:
    ...
    @property
    def isPassive(self):
        """Whether density is zero and passive attenuation is positive."""
        return not self.isActive and float(self.bulkAttenuation.toValue(units.m**-1)) > 0.0

    @property
    def isTransparent(self):
        """Whether both active-ion density and passive attenuation are zero."""
        return not self.isActive and float(self.bulkAttenuation.toValue(units.m**-1)) == 0.0
...
class Material:
    ...
    :class:`MaterialCondition` before registering it on a simulation.
    """
    ...
    def __init__(self, name: str, *, metadata: Mapping[str, Any] | None = None):
        ...
        self._states: list[MaterialState] = []

    @property
    def states(self):
        """Temperature-ordered immutable tuple of :class:`MaterialState` values."""
    ...
    def addState(
        ...
        *,
        ...
        crossSections=None,
        bulkAttenuation=Quantity(0.0, units.m**-1),
        metadata=None,
    ):
        ...
        bulkAttenuation
            Non-negative inverse-length passive loss coefficient.
        metadata
    ...
    def _append_state(self, state: MaterialState):
        if not isinstance(state, MaterialState):
            raise TypeError("state must be MaterialState")
    ...
    def at(
        ...
        *,
        ...
        -------
        MaterialCondition
            Immutable condition ready for :meth:`Simulation.addMaterial`.
        """
    ...
    def toHdf5(self, path, *, key=None, overwrite=False):
        """Write this material as a one-entry HDF5 material library.
        ...
        key
            Optional database key; defaults to an identifier derived from the
            material name.
        overwrite
            Replace an existing file only when true.
        """
        from .library import MaterialLibrary

        library = MaterialLibrary()
        library.register(key or _default_key(self.name), self)
        library.toHdf5(path, overwrite=overwrite)

    @classmethod
    def fromHdf5(cls, path, *, key=None):
        """Load one material from a versioned HDF5 material library.
        ...
        path
            Source HDF5 path.
        key
            Entry key, required when the file contains multiple materials.
        """
        from .library import MaterialLibrary
