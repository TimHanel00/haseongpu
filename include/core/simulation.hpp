...
namespace hase::core
{
            ...
            hase::data::TraceData& hostMesh)
        {
            ...
            auto i = alpaka::onHost::executeForEachIfHasDevice(
                [&](auto const& backend) -> int
                {
                    ...
                    if(verbosity & V_STAT)
                    {
                        ...
                        dout(V_STAT) << "Samples           : "
                                     << std::min(static_cast<unsigned>(result.dndtAse.size()), numSamples)
                                     << std::endl;
                        ...
                        {
                            dout(V_STAT)
                                << "Forward rays      : " << rayCount << " of " << experiment.minRays << " - "
                        ...
                        dout(V_STAT) << "sum(totalRays)    : "
                                     << std::accumulate(result.totalRays.begin(), result.totalRays.end(), 0.)
                                     << std::endl;
                        dout(V_STAT) << "RSE threshold     : " << experiment.relativeStandardErrorThreshold
                                     << std::endl;
                        dout(V_STAT) << "Spectral samples  : " << hostMesh.crossSectionWavelengths.size() << std::endl;
                        ...
                        dout(V_STAT)
                            << "=== Adaptive forward-ray convergence by cell (green: RSE target reached; red: "
                               "budget exhausted) ==="
                            << std::endl;
            ...
            if(!oneDidRun)
            {
                std::cout << "\n------------------------------ HASEONGPU ERROR ------------------------------\n"
                          << std::endl;
                std::cout
                    << " Backend did not match any available backend with available device! \n Available backend "
                       "specifications are: "
                    << std::endl;
    ...
    /**
     * @brief Run one ASE solve directly from the public primitive graph.
     *
