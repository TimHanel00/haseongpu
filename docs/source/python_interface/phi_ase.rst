...
``repetitions``
   Maximum number of repeated PhiASE runs using the same ray count if the MSE
   target is not reached. Since the importance-sampling distribution assigns
   rays to prisms stochastically, repeating the calculation can improve the
   estimate without increasing the number of rays per run.

``adaptiveSteps``
   Number of ray-count increases between ``minRaysPerSample`` and
   ``maxRaysPerSample`` when the MSE threshold is not reached.

``useReflections``
   Enables or disables reflections at the top and bottom surfaces.

``monochromatic``
   Restricts the PhiASE calculation to the first absorption and emission
...
``nPerNode``
   ...
   not serialized as a HASE openPMD schema attribute. See :doc:`../mpi` and
...
for the schema, storage backend options, artifact-retention environment
