...
class GainMedium:
    ...
    def defineField(self, name, *, entity, values, dtype=None, unit="1", dynamic=False, backendRequired=False):
        ...
        if canonical in PHYSICAL_PROPERTY_SPECS or name in _CUSTOM_FIELD_RESERVED_NAMES:
            raise ValueError(f"field name '{name}' is reserved by the built-in gain-medium schema")
    ...
    def _schemaFieldSpecs(self, schema):
        ...
        if hasattr(schema, "fieldSpecs"):
            return schema.fieldSpecs()
        raise TypeError("schema must be a PrimitiveSchema or PrimitiveSchemaDefinition subclass")
